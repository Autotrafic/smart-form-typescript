...
const Label = styled.label`
  cursor: pointer;
  display: block;
  border: ${({ $fileAttached }) => ($fileAttached ? `1px solid ${colors.primaryColor}` : `1px solid ${colors.primaryColor}`)};
  text-align: center;
  color: ${colors.black};
      ...
      <InputContainer>
        <FileInputConainer $fileAttached={file && file.name}>
          ...
          <Label $fileAttached={file && file.name} htmlFor={id}>
            <FileInputButton $fileAttached={file && file.name}>{file?.name || 'Hacer foto o seleccionar archivo'}</FileInputButton>
          </Label>
        </FileInputConainer>
        {file && (
