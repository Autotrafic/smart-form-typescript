...
function SummaryModalContent({ orderData }: { orderData: IOrder }) {
  const car = VehicleType.CAR;
  const motorbike = VehicleType.MOTORBIKE;
  const {itp: itpData} = orderData;
  const { vehicle } = orderData.vehicleForm;
  const { value: motorbikeValue } = vehicle;
  const { value: carValue } = vehicle.model;
  ...
  let vehicleValue;
  if (vehicle.type === car) {
    vehicleSelected = modelName ? `${brand} ${modelName}` : '---';
    vehicleValue = carValue ?? '---';
  } else if (vehicle.type === motorbike && motorbikeValue === CICLOMOTOR_VALUE) {
    vehicleSelected = 'Ciclomotor';
    vehicleValue = motorbikeValue ?? '---';
  } else if (vehicle.type === motorbike && motorbikeValue !== CICLOMOTOR_VALUE) {
    vehicleSelected = 'Motocicleta';
    vehicleValue = motorbikeValue ?? '---';
  }
  ...
  const buyerCommunity = `% ITP ${getCommunityByCode(orderData.vehicleForm.buyerCommunity)}`;

  const breakdownItems = [
    { title: 'Vehículo', content: vehicleSelected },
  ...
  return (
    <Container>
      ...
      <ContentWrapper>
        <Description>
          El Impuesto de Transmisiones Patrimoniales (ITP) es un gravamen que debe ser pagado por el adquirente al momento de
          realizar la transferencia de propiedad de un vehículo de un particular a otro.
        </Description>
        <Description>
          Este impuesto se determina mediante la aplicación de una tasa, que difiere dependiendo de la comunidad autónoma en
          la que reside el comprador, al valor que resulte mayor entre el convenido en la compraventa y el valor venal del
          automóvil, establecido en las tablas proporcionadas por la Agencia Tributaria.
        </Description>
      </ContentWrapper>
      ...
      <ContentWrapper>
        <TaxContainer>
          <Description>
            ...
            <NoWrapText>( % Depreciación ) X </NoWrapText> <NoWrapText>( % ITP CCAA ) = </NoWrapText>
            <b>ITP</b>
          </Description>
        </TaxContainer>
        <Breakdown>
