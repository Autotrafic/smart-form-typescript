  ...
  useEffect(() => {
    ...
    if (value.date.month && value.date.year) {
      ...
      if (!days.some((day) => day.value === Number(value.date.day))) {
        updateFormData((prev) =>
          prev ? { ...prev, date: { ...prev.date, day: "" } } : { day: "" }
        );
      }
    }
    ...
    <div>
      ...
      <Container>
        <Dropdown
          ...
          handleChange={(value) =>
            updateFormData((prev) =>
              prev ? { ...prev, date: { ...prev.date, year: value } } : { year: value }
            )
          }
          {...props}
        />
        <Dropdown
          ...
          handleChange={(value) =>
            updateFormData((prev) =>
              prev ? { ...prev, date: { ...prev.date, month: value } } : { month: value }
            )
          }
          fontSize="12px"
          {...props}
        ...
        <Dropdown
          ...
          handleChange={(value) =>
            updateFormData((prev) =>
              prev ? { ...prev, date: { ...prev.date, day: value } } : { day: value }
            )
          }
          {...props}
        />
